...
def save_annotations(image_path, scale_x, scale_y):
    ...
    print(f"[SAVED] JSON saved to: {json_path}")
    ...
    if original_img is None:
        print("[ERROR] Could not reload original image for saving annotated image.")
        return
    ...
    print(f"[SAVED] Preview saved to: {output_path}")
...
def annotate_image(image_path):
    ...
    while True:
        ...
        if key == 27:  # Escape key - quit entire process
            print("\n[EXIT] Quitting annotation tool...")
            ...
            print(f"[OK] Saved polygon: {current_class}")
            points.clear()
        elif key in key_map:
...
def annotate_folder(folder_path):
    ...
    if os.path.exists(labels_dir):
        annotated_names = {os.path.splitext(f)[0] for f in os.listdir(labels_dir) if f.lower().endswith(".json")}

    pending = [(i, f) for i, f in enumerate(image_files, 1) if os.path.splitext(f)[0] not in annotated_names]
    print(f"Found {total} images in {folder_path} ({total - len(pending)} already done, {len(pending)} remaining)")

    for i, filename in pending:
        print(f"\n[>>] [{i}/{total}] Annotating: {filename}")
        result = annotate_image(os.path.join(folder_path, filename))
        if result == "EXIT":
            return "EXIT"


def annotate_all_folders(base_path):
    """Iterate through all subfolders in base_path, skipping fully annotated ones."""
    if not os.path.exists(base_path):
        print(f"[ERROR] Error: Base folder not found at {base_path}")
        return
    ...
    # Filter out fully annotated folders
    pending_folders = []
    for folder_name in subfolders:
        folder_full = os.path.join(base_path, folder_name)
        images = [f for f in os.listdir(folder_full) if f.lower().endswith((".jpg", ".jpeg", ".png", ".bmp"))]
        labels_dir = os.path.join(folder_full, "labels")
        annotated = len([f for f in os.listdir(labels_dir) if f.lower().endswith(".json")]) if os.path.exists(labels_dir) else 0
        if annotated < len(images):
            pending_folders.append(folder_name)

    skipped = total_folders - len(pending_folders)
    print(f"[FOLDER] Found {total_folders} folders in {base_path} ({skipped} fully annotated, skipped)")
    print(f"[FOLDER] {len(pending_folders)} folders remaining to annotate")
    print("Controls: 's' = save & next | 'q' = skip image | Esc = quit entirely\n")

    for i, folder_name in enumerate(pending_folders, 1):
        folder_full = os.path.join(base_path, folder_name)

        print(f"\n{'='*60}")
        print(f"[FOLDER] [{i}/{len(pending_folders)}] Starting folder: {folder_name}")
        print(f"{'='*60}")
        result = annotate_folder(folder_full)
        if result == "EXIT":
            print("\n[EXIT] Annotation stopped by user.")
            return
        print(f"[OK] Done with folder: {folder_name}")


# [FOLDER] UPDATE BASE PATH HERE
